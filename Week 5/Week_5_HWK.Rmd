...
RepFunc <- sapply(funs, is.primitive)
names(RepFunc)

PrimFunc <- names(RepFunc[RepFunc==TRUE])
PrimFunc
```
...
```{r}
# test number
t1 <- c(1,15,13,1,2,3,4,5,55)
t2 <- c(1,4,55,66,88,99)
...
```{r}
`random.replace<-` <- function(x, value){
  x[sample(length(x), 1)] <- value
  x
}

test <- 1:5
random.replace(test) <- 25
test
```
...
When using in_dir you can set the working directory to change back to the original using the on.exit paramater.  
...
```
Capture.output2 is much shorter than capature.output. This makes capture.output2 easier to read but it could potentially sacrifice functionality.
...
```{r }
setwd("/Users/qihao/OneDrive/?????????/PLS900")
